...
var NewSectionComponent = (function () {
    ...
    NewSectionComponent.prototype.deleteSection = function () {
        ...
        if (this.section) {
            ...
            var bodyString = JSON.stringify(delSection);
    ...
    NewSectionComponent.prototype.uploadImage = function (newSection) {
        ...
        if (fileCount > 0) {
            ...
            this.http
                .post('section/uploadImage/' + newSection.id, formData).map(function (res) { return res.json(); }).subscribe(function (success) {
    ...
    NewSectionComponent.prototype.onSubmit = function () {
        ...
        if (this.form.valid) {
            ...
            if (this.editMode) {
                var bodyString = JSON.stringify(section);
                var headers = new http_1.Headers({ 'Content-Type': 'application/json' });
                var options = new http_1.RequestOptions({ headers: headers });
                this.http.put('section/' + this.section.id, bodyString, options)
                    .map(function (res) { return res.json(); })
